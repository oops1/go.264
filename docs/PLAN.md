...
amd64 assembly for sum of absolute differences, the forward and inverse
4x4 transforms, quantisation, dequantisation and the residual add, all
generated by an avo program in its own nested module so the library keeps
a dependency free go.mod. CI regenerates and diffs the assembly. The
16x16 block matching kernel is about thirty times faster than Go and the
transform kernels two to five times, but neither moves the whole encode
by more than a few per cent: the time is in the mode decision, not the
arithmetic. Still to cover: transformed differences, the six tap
interpolation and the deblocking edge filters.
...
Done for the adapters this project can reach. `internal/hwaccel/mf` reaches Media
...
### Phase 12 — Release
Done. v1.0.0 tagged the Constrained Baseline codec; v1.1.0 tagged Main
